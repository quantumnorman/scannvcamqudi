...
class APDMainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        ui_file = os.path.join(this_dir, 'apdgui.ui')

        # Load it
        print("super")
        super(APDMainWindow, self).__init__()
        uic.loadUi(ui_file, self)
        self.show()

class APDGui(GuiBase ):

    # declare connectors
    apd_logic = Connector(interface='APDLogic')
    ...
    def on_activate(self):
        ...
        self.apdlogic = self.apd_logic()

        self.apdlogic.sigAPDUpdated.connect(self.update)

        # Set slider to appropriate range
        v_range = self.apdlogic.volt_range

        self._mw.output_adj.setMinimum(v_range[0]*10)
        self._mw.output_adj.setMaximum(v_range[1]*10)

        self.apdlogic.start_poll()
    ...
    def on_deactivate(self):
        """ Deactivate module
        """
        self.apdlogic.stop_poll()

    def update(self, param_dict):
        """
        Callback to update interface when the APD logic produces a sigAPDUpdated.
        @param dict param_dict: Dict of parameters to use for updating GUI.
        """
        try:
            # Get photodiode voltage and power
            counts = float(param_dict['counts'])



            # Update readout widgets
            self._mw.voltage_readout.setText("{:.3f}".format(counts))
            self._mw.power_readout.setText("{:.3f}".format(counts))
            self._mw.aom_out.setText("{:.2f}".format(counts))

            # Add power to rolling buffer and update plot data
            self.power_buffer = np.roll(self.power_buffer, -1)
            self.power_buffer[-1] = counts
        except:
