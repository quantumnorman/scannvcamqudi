...
class APDGui(GuiBase):
    ...
    def _apply_trace_view_settings(self, setting):
        active_channels, averaged_channels = self.apd_logic().channel_settings
    ...
    def update_channel_settings(self, enabled, averaged):
        ...
        self._streamer_constraints = self.apd_logic().streamer_constraints
        ...
        if channel and channel != 'None':
            try:
                if channel.startswith('average '):
                    ...
                    val = data[channel][-1]
                constraints = self.apd_logic().streamer_constraints
    ...
    def _restore_default_view(self):
        ...
        self.update_status(running=self.apd_logic().module_state() == 'locked',
                           recording=self.apd_logic().data_recording_active)
