...
class CounterGui(GuiBase):
    ...
    def on_activate(self):
        ...
        self._pw.setXRange(
            0,
            self._counting_logic._count_length
        )
    ...
    def updateData(self):
        ...
        if self._counting_logic.module_state() == 'locked':
            ...
            x_vals = (
                np.arange(0, self._counting_logic._countlength))

            ymax = -1
            ymin = 2000000000
    
            self.curves[0].setData(y=self._counting_logic.countdata, x=x_vals)
            #self.curves[2 * i + 1].setData(y=self._counting_logic.countdata_smoothed[i],
            #                               x=x_vals
            #                               )
            if ymax < self._counting_logic.countdata.max() and self._trace_selection:
                ymax = self._counting_logic.countdata.max()
            if ymin > self._counting_logic.countdata.min() and self._trace_selection:
                ymin = self._counting_logic.countdata.min()
